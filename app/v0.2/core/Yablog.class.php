...
class Yablog {
     ...
     */
    public function run() {
        $class = A(CONTROLLER_NAME);//实例化

        if (method_exists($class, $action = ACTION_NAME . 'Action') || method_exists($class, '__call')) {
            method_exists($class, 'initialize') && $class->initialize();
            call_user_func(array($class, $action));
        }
        else {
            throw new Exception(L('METHOD_NOT_EXIST'));
        }
    }
}